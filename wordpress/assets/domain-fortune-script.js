(function($) {
    ...
    $(document).ready(function() {
        const $domainForm = $('#fatefinder-domain-form');
        const $numberForm = $('#fatefinder-number-form');
        const $domainLoading = $('#fatefinder-domain-loading');
        const $numberLoading = $('#fatefinder-number-loading');
        const $domainResult = $('#fatefinder-domain-result');
        const $numberResult = $('#fatefinder-number-result');
        const $domainInput = $('#domain-input');
        const $numberInput = $('#number-input');
        const $domainButton = $('#fatefinder-domain-btn');
        const $numberButton = $('#fatefinder-number-btn');
        
        let isProcessingDomain = false;
        ...
        $domainForm.on('submit', function(e) {
            ...
            if (isProcessingDomain) return;
            
            const domain = $domainInput.val().trim();
            
            if (!domain) {
                showError($domainResult, 'Please enter a domain name.');
                return;
            }
            
            isProcessingDomain = true;
            $domainButton.prop('disabled', true);
            $domainForm.hide();
            $domainResult.hide();
            $domainLoading.show();
            ...
            setTimeout(function() {
                try {
                    ...
                    displayAnimatedText($domainResult, `Domain: ${domain}`, `Fortune Result: ${fortuneMessage}`);
                } catch (error) {
                    showError($domainResult, 'An error occurred. Please try again.');
                    console.error('Domain fortune calculation error:', error);
                } finally {
                    isProcessingDomain = false;
                    $domainButton.prop('disabled', false);
                    $domainLoading.hide();
                    $domainForm.show();
                    $domainResult.show();
                }
        ...
        $numberForm.on('submit', function(e) {
            ...
            if (isProcessingNumber) return;
            
            const number = $numberInput.val().replace(/\D/g, ''); // Remove non-numeric characters
            
            if (!number) {
                showError($numberResult, 'Please enter a valid number.');
                return;
            }
            ...
            setTimeout(function() {
                try {
                    ...
                    const fortuneMessage = getFortuneMessage(fortuneScore);
                    displayAnimatedText($numberResult, `Number: ${number}`, `Fortune Result: ${fortuneMessage}`);
                } catch (error) {
                    showError($numberResult, 'An error occurred. Please try again.');
                    console.error('Number fortune calculation error:', error);
                } finally {
        ...
        // Enter key support
        $domainInput.on('keydown', function(event) {
            if (event.key === 'Enter') {
                $domainForm.submit();
            }
        });
        ...
        /**
         * Calculate domain fortune score using weighted frequency method
         */
        ...
        /**
         * Calculate number fortune score
         */
        ...
        /**
         * Get fortune message based on score (complete 81 results)
         */
        function getFortuneMessage(score) {
            switch (score) {
                ...
                default:
                    return "Unknown fortune result. Please try again.";
            }
        }
        
        /**
         * Display animated text results
         */
        function displayAnimatedText($element, mainText, fortuneText) {
            $element.html('');
            
            let html = '<div class="fatefinder-result-name">' + escapeHtml(mainText) + '</div>';
            html += '<div class="fatefinder-result-fortune">' + escapeHtml(fortuneText) + '</div>';
        ...
        function showError($element, message) {
            $element.html('<div class="fatefinder-error">' + escapeHtml(message) + '</div>');
            $element.show();
        }
